...
```bash
FIGMA_ACCESS_TOKEN=\"$FIGMA_ACCESS_TOKEN\" npx figma connect publish
```

Resolve `FIGMA_ACCESS_TOKEN` from an approved secret manager or local
environment without printing it. Never paste a token into chat or commit it.
