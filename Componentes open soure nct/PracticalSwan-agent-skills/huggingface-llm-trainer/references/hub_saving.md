...
### Method 2: Approved environment token

```python
"secrets": {"HF_TOKEN": "$HF_TOKEN"}
```

Resolve `$HF_TOKEN` from the approved local secret or job secret store. Never
hardcode, paste, or print the token value.
...
Use the environment only when the host's approved secret handling supplies the
value; do not put a literal token in source or command arguments.
