...
### Authentication — check first, use a safe handoff if needed

Don't ask for a token reflexively. Check whether the user is already
authenticated, and if there is no usable session, stop for a local login or
approved secret-manager handoff. Never request or accept a token value in chat.
...
- **User already authenticated and the LoRA repo is private**: try `api.repo_info(repo_id, token=cached)`. If it succeeds, the existing token has the right scope — proceed. If it fails (token doesn't have access to that repo), ask the user to refresh the approved local secret or login with broader access.
- **No cached token**: stop and ask the user to complete `hf auth login` locally or configure an approved secret manager with write scope. Do not request the token value.
...
- **`HfHubHTTPError: 401/403` on `upload_file`.** Token lacks write scope. Fix: ask the user to refresh the approved local secret or login with a fine-grained token that has write permission on this specific Space; do not request the value.
