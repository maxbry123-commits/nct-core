...
For Atlas M10+ use slow query logs to find the slowest aggregations. See [Slow query logs](source-slow-query-logs.md). 
Use codebase if available, ask the user.

### Check if same aggregation runs repeatedly

For Atlas M10+ use $queryStats. See [Query Stats](source-query-stats.md)
Use codebase if available, ask the user.
