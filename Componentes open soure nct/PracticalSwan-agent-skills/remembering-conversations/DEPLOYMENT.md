...
ls -l ~/.claude/tool/index-conversations
...
**Fix:**
```bash
# Set the API key through the shell or host's secure secret mechanism without
# pasting it into logs, documentation, or committed files.
...
# If still failing, inspect the exact model-cache path. Clear only the verified
# ~/.cache/transformers directory after explicit approval, then rerun:
./index-conversations
```
...
**For searching conversations from within Claude Code sessions**, use the subagent pattern (see `using-skills` for complete workflow).
