...
```bash
~/.claude/tool/install-hook
```

**Index all conversations:**
```bash
~/.claude/tool/index-conversations
```

**Process unindexed only:**
```bash
~/.claude/tool/index-conversations --cleanup
```
...
**Summaries failing:**
- Check only whether the API key is set: `if [ -n "${ANTHROPIC_API_KEY:-}" ]; then echo "ANTHROPIC_API_KEY is set"; else echo "ANTHROPIC_API_KEY is missing"; fi`
