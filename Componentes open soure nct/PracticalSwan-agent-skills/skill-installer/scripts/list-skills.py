...
def _installed_skills(installed_root: str | None = None) -> set[str]:
    root = os.path.expanduser(installed_root) if installed_root else os.path.join(_codex_home(), "skills")
...
def _parse_args(argv: list[str]) -> Args:
    ...
    parser.add_argument(
        "--installed-root",
        help="Skill root used for installed annotations (defaults to Codex skills).",
    )
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        installed = _installed_skills(args.installed_root)
        if args.format == "json":
            payload = [
