...
INDEX_PATH = pathlib.Path(os.getenv("INDEX_PATH", "📂componentes open soure nct-core/README-INDICE-COMPONENTES.md")).expanduser()
...
def run_item(item):
    ...
    env.update({
        ...
        "DEST_REPO": item.get("dest_repo", env.get("DEST_REPO", "maxbry123-commits/nct-core")),
        "DEST_BRANCH": item.get("dest_branch", env.get("DEST_BRANCH", "main")),
        "DEST_ROOT": item.get("dest_root", env.get("DEST_ROOT", "📂componentes open soure nct-core")),
